   ...
   <!-- component -->
    ...
    <div class="max-w-2xl mx-auto">
      <div class="flex items-center justify-center w-full">
            <label v-if="!fileUpload.name" for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
      ...
      <div class="flex gap-2 flex-row-reverse mt-5 text-center w-full">
        <button type="button" class="text-white px-4 py-3 rounded-xl flex" :class="fileUpload.name ? 'bg-blue-400' : 'bg-blue-300 cursor-default'">Simpan</button>
        <button type="button" class="text-white px-4 py-3 rounded-xl flex" :class="fileUpload.name ? 'bg-red-400' : 'bg-red-300 cursor-default'" :disabled="!fileUpload.name" @click="reset()">Batal</button>
      </div>
    </div>
  </div>
</template>