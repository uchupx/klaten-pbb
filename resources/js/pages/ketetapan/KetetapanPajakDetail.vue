      ...
      <table class="table table-fixed w-full text-xs">
        ...
        <template v-if="list.length > 0" >
          <tr class="border-b" v-for="item in list" :key="item.nop">
            ...
            <td class="w-10 py-2"> {{item.nop ? item.nop : "-" }} </td>
            <td class="w-10 py-2"> {{item.tahun_pajak ? item.tahun_pajak : "-" }} </td>
            <td class="w-10 py-2"> {{item.nama_wp ? item.nama_wp : "-" }} </td>
            <td class="w-10 py-2"> {{item.alamat_wp ? item.alamat_wp : "-" }} </td>
            <td class="w-10 py-2"> {{item.alamat_op ? item.alamat_op : "-" }} </td>
            <td class="w-10 py-2"> {{item.luas_bumi ? item.luas_bumi : "-" }} </td>
            <td class="w-10 py-2"> {{item.luas_bangunan ? item.luas_bangunan : "-" }} </td>
            <td class="w-10 py-2"> {{item.njop_bumi ? item.njop_bumi : "-" }} </td>
            <td class="w-10 py-2"> {{item.njop_bangunan ? item.njop_bangunan : "-" }} </td>
            <td class="w-10 py-2"> {{item.njop_sppt ? item.njop_sppt : "-" }} </td>
            <td class="w-10 py-2"> {{item.total_pajak ? item.total_pajak : "-" }} </td>
          </tr>
        </template>
      </table>
      <template v-if="list.length == 0">
